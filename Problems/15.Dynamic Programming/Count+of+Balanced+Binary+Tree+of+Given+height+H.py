...
def count_balanced_bts_recursive(h, level=0):
    # Base Case
    indent = "  " * level
    print(f"{indent}count_balanced_bts_recursive({h}) called")
    if (h == 0 or h == 1):
        print(f"{indent}Return 1 (base case)")
        return 1   # For 0, we will say that a single tree of value None is possible

    h1 = count_balanced_bts_recursive(h-1, level+1)
    h2 = count_balanced_bts_recursive(h-2, level+1)

    ans = h1*h1 + 2*h1*h2
    print(f"{indent}Return {ans} (h={h}, h1={h1}, h2={h2})")
    return ans
